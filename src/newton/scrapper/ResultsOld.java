...
public class ResultsOld {
    ...
    static OkHttpClient fetc = new OkHttpClient();

    public static boolean lookUP(int roll, int year) {
     ...
     * @param end
     */
    public ResultsOld(int year, int start) {
        this.year = year;
        this.start = start;
    ...
    public static File getPath(String roll, int year) {
    ...
    private static void split(int JUMP, int end, int s, int year) {
        ...
        Arrays.stream(toArray).map(CompletableFuture::join).filter(ob -> ob != null).forEach(ResultsOld::doRange);
